...
const QVector<QByteArray> &SimpleDeframer<SYNC_T, SYNC_SIZE, FRAME_SIZE, ASM_SYNC>::work(const QByteArray &data)
{
  // Output buffer
  framesOut.clear();

  // Loop in all bytes
  for (const char &cbyte : data)
  {
    uint8_t byte = static_cast<uint8_t>(cbyte);
    // Loop in all bits!
    for (int i = 7; i >= 0; i--)
    {
      // Get a bit, push it

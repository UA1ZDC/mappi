...
void parseStream(const QString& fileName, const QString& weatherFile, const QString& path, TSatPretr::PretrOpt& opt)
{
  ...
  if (opt.headerExist) {
    ...
    handler.setSite("Санкт-Петербург");
  }

  //handler.setNameSuffix("win");
...
void readConfig(QString* weatherFile, QString* path)
{
  ...
  QDateTime dt = QDateTime::currentDateTimeUtc();
  *weatherFile = meteo::global::findWeatherFile(pbtools::toQString(conf.file_storage().tle()), dt);
...
int main(int argc, char** argv)
{
  ...
  // if( !::mappi::inter::Settings::instance()->load() ){
  //   return EXIT_FAILURE;
  // }  
  ...
  if (optind >= argc) {
    info_log << QObject::tr("В качестве параметров надо задать полное имя raw-файла. (\"test -h\" для помощи)");
    return -1;
  } 

  QString fileName = argv[optind];

  debug_log << opt.manchester << opt.deframer << opt.invert << opt.headerExist << opt.oldHeaderExist;

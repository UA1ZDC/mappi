...
namespace {
  mappi::to::ThemAlg* createkSeaTempr(mappi::conf::ThemType, QSharedPointer<mappi::to::DataStore>& ds)
  {
    return new mappi::to::kSeaTempr(ds);
  }

  static const bool res = mappi::to::singleton::ThemFormat::instance()->registerThemHandler(mappi::conf::kSeaTempr, createkSeaTempr);
}

using namespace mappi;
using namespace to;

kSeaTempr::kSeaTempr(QSharedPointer<to::DataStore>& ds):
  ThemAlg(ds)
{
...
bool kSeaTempr::readConfig(QList<conf::InstrumentType>* valid)
{
  return ThemAlg::readConfig(CONF_FILE, &_conf, valid);
}

//! список псевдонимов каналов необходимых для обработки
bool kSeaTempr::channelsList(conf::InstrumentType instr, QList<uint8_t>* chl)
{
...
bool kSeaTempr::process(conf::InstrumentType instr)
{
  _data.clear();

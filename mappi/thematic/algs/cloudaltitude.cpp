...
namespace {
  mappi::to::ThemAlg* createCloudAltitude(mappi::conf::ThemType, QSharedPointer<mappi::to::DataStore>& ds)
  {
    return new mappi::to::CloudAltitude(ds);
  }

  static const bool res =
      mappi::to::singleton::ThemFormat::instance()->registerThemHandler(mappi::conf::kCloudAlt, createCloudAltitude);
}

using namespace mappi;
using namespace to;

CloudAltitude::CloudAltitude(QSharedPointer<to::DataStore>& ds):
  ThemAlg(ds)
{
...
bool CloudAltitude::readConfig(QList<conf::InstrumentType>* valid)
{
  return ThemAlg::readConfig(CONF_FILE, &_conf, valid);
}

//! список псевдонимов каналов необходимых для обработки
bool CloudAltitude::channelsList(conf::InstrumentType instr, QList<uint8_t>* chl)
{
...
bool CloudAltitude::process(conf::InstrumentType instr)
{
  _data.clear();

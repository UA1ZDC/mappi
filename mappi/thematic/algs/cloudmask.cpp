...
#define CONF_FILE  MnCommon::etcPath("mappi") + "/thematics/cloudmask.conf"


namespace {
  mappi::to::ThemAlg* createCloudMask(mappi::conf::ThemType, QSharedPointer<mappi::to::DataStore>& ds)
  {
    return new mappi::to::CloudMask(ds);
  }

  static const bool res = mappi::to::singleton::ThemFormat::instance()->registerThemHandler(mappi::conf::kCloudMask, createCloudMask);
}

using namespace mappi;
using namespace to;

CloudMask::CloudMask(QSharedPointer<to::DataStore>& ds):
  ThemAlg(ds)
{
...
bool CloudMask::readConfig(QList<conf::InstrumentType>* valid)
{
  return ThemAlg::readConfig(CONF_FILE, &_conf, valid);
}

//! список псевдонимов каналов необходимых для обработки
bool CloudMask::channelsList(conf::InstrumentType instr, QList<uint8_t>* chl)
{
...
bool CloudMask::process(conf::InstrumentType instr)
{
  _data.clear();

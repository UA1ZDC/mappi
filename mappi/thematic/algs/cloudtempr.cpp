...
namespace {
  mappi::to::ThemAlg* createCloudTempr(mappi::conf::ThemType, QSharedPointer<mappi::to::DataStore>& ds)
  {
    return new mappi::to::CloudTempr(ds);
  }

  static const bool res = mappi::to::singleton::ThemFormat::instance()->registerThemHandler(mappi::conf::kCloudTempr, createCloudTempr);
}

using namespace mappi;
using namespace to;

CloudTempr::CloudTempr(QSharedPointer<to::DataStore>& ds):
  ThemAlg(ds)
{
...
CloudTempr::~CloudTempr()
{
}


bool CloudTempr::readConfig(QList<conf::InstrumentType>* valid)
{
  return ThemAlg::readConfig(CONF_FILE, &_conf, valid);
}

//! список псевдонимов каналов необходимых для обработки
bool CloudTempr::channelsList(conf::InstrumentType instr, QList<uint8_t>* chl)
{
...
bool CloudTempr::process(conf::InstrumentType instr)
{
  _data.clear();

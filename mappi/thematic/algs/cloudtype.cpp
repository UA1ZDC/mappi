...
namespace {
  mappi::to::ThemAlg* createCloudType(mappi::conf::ThemType, QSharedPointer<mappi::to::DataStore>& ds)
  {
    return new mappi::to::CloudType(ds);
  }

  static const bool res = mappi::to::singleton::ThemFormat::instance()->registerThemHandler(mappi::conf::kCloudType, createCloudType);
}

using namespace mappi;
using namespace to;

CloudType::CloudType(QSharedPointer<to::DataStore>& ds):
  ThemAlg(ds)
{
...
bool CloudType::readConfig(QList<conf::InstrumentType>* valid)
{
  return ThemAlg::readConfig(CONF_FILE, &_conf, valid);
}

//! список псевдонимов каналов необходимых для обработки
bool CloudType::channelsList(conf::InstrumentType instr, QList<uint8_t>* chl)
{
...
bool CloudType::process(conf::InstrumentType instr)
{
  _data.clear();

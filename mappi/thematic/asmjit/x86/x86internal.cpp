  ...
  if (cc.getAlgorithm() == CallConv::kAlgorithmDefault) {
    ...
    for (i = 0; i < argCount; i++) {
      ...
      if (TypeId::isInt(typeId)) {
        uint32_t regId = gpzPos < CallConv::kNumRegArgsPerKind ? cc._passedOrder[X86Reg::kKindGp].id[gpzPos] : (uint8_t)Globals::kInvalidRegId;
      ...
      if (TypeId::isFloat(typeId) || TypeId::isVec(typeId)) {
        uint32_t regId = vecPos < CallConv::kNumRegArgsPerKind ? cc._passedOrder[X86Reg::kKindVec].id[vecPos] : (uint8_t)Globals::kInvalidRegId;
  ...
  if (cc.getAlgorithm() == CallConv::kAlgorithmWin64) {
    for (i = 0; i < argCount; i++) {
      ...
      if (TypeId::isInt(typeId) || TypeId::isMmx(typeId)) {
        uint32_t regId = i < CallConv::kNumRegArgsPerKind ? cc._passedOrder[X86Reg::kKindGp].id[i] : (uint8_t)Globals::kInvalidRegId;
        if (regId != Globals::kInvalidRegId) {
      ...
      if (TypeId::isFloat(typeId) || TypeId::isVec(typeId)) {
        uint32_t regId = i < CallConv::kNumRegArgsPerKind ? cc._passedOrder[X86Reg::kKindVec].id[i] : (uint8_t)Globals::kInvalidRegId;

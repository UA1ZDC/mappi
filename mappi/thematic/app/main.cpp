...
//bool getThemConf(const QString& satname, QList<conf::ThemType>* typelist)
//{
//  conf::ThematicConf conf;
...
//  for(auto them : conf.thematics()) {
//    if (satname == QString::fromStdString(them.satellite())) {
//  debug_log <<  them.Utf8DebugString();
//      for (auto type: them.type()) {
//  typelist->append(conf::ThemType(type));
//      }
//      break;
//    }
//    debug_log << satname << QString::fromStdString(them.satellite()) ;
//  }
...
void dataProcess(const QDateTime& start, const QString& satname, const QList<conf::InstrumentType>& /*instr*/)
{
...
//    var(type);
//    auto alg = std::unique_ptr<to::ThemAlg>(to::singleton::ThemFormat::instance()->createThemAlg(type, store));
//    if (nullptr == alg) {
//      continue;
//    }
//    alg->process(start, satname, instr);
//    var("");
//  }

...
/*
    ...
    if(nullptr != channel)
    {
      conf::ScheduleReply* response = channel->remoteCall(&mappi::conf::ScheduleService::GetSchedule, Dummy(), 10000, true);
      if(nullptr != response)
      {
...
/*
    ...
    {
      conf::ScheduleReply* response =
          channel->remoteCall(&mappi::conf::ScheduleService::Refresh, Dummy(), 10000, true);
      if(nullptr != response)
      {
        if(response->has_result()) {
          ok = response->result();
...
/*
    ...
    if(nullptr != channel)
    {
      conf::AllSatellitesReply* response =
          channel->remoteCall(&mappi::conf::ScheduleService::GetAllSatellites, Dummy(), 10000, true);
      if(nullptr != response)
      {

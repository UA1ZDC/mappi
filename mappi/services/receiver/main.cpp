...
int main(int argc, char *argv[])
{
  ...
  receiver::ServiceHandler service;
  if (service.init() == false)
    return EXIT_FAILURE;

  try {
    receiver::ServerStub* stub = new receiver::ServerStub(&service);
    int port = global::service(settings::proto::kReceiver).port();
    rpc::Server* server = new rpc::Server(stub, QString("0.0.0.0:%1").arg(port));
    if (server->init() == false)
      return EXIT_FAILURE;

    info_log << QObject::tr("%1 запущен").arg(global::serviceTitle(settings::proto::kReceiver));

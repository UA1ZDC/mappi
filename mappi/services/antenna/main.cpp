...
int main(int argc, char *argv[])
{
  ...
  antenna::ServiceHandler service;
  if (service.init() == false)
    return EXIT_FAILURE;

  try {
    antenna::ServerStub* stub = new antenna::ServerStub(&service);
    int port = global::service(settings::proto::kAntenna).port();
    rpc::Server* server = new rpc::Server(stub, QString("0.0.0.0:%1").arg(port));
    if (server->init() == false)
      return EXIT_FAILURE;

    // отписка
    QObject::connect(server, &rpc::Server::clientUnsubscribed, &service, &antenna::ServiceHandler::unsubscribe);

    info_log << QObject::tr("%1 запущен").arg(global::serviceTitle(settings::proto::kAntenna));

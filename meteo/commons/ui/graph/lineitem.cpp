...
LineItem::LineItem(map::LayerItems* layer, QGraphicsItem* parent, QGraphicsScene* scene, LineType type)
...
QRectF LineItem::boundingRect() const
{
  ...
  QRectF r(-2,-2,4,4);
  switch (type_) {
    case LineType::kGraph:
      r.setTop(mapRect.top() - p.y());
      r.setBottom(mapRect.bottom() + p.y());
    break;
    case LineType::kOcean:
      r.setLeft(mapRect.left()-p.x());
      r.setRight(mapRect.right() + p.x());
    break;
    default:
    break;
  }

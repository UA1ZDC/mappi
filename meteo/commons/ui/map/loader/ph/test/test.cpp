...
int main( int argc, char* argv[] )
{
  ...
  meteo::map::Map* map = new meteo::map::Map( meteo::GENERAL );
  bool res = meteo::map::Loader::instance()->handleData( "ph.load", map );
  if ( false == res ) {
    error_log << QObject::tr("Не удалось згрузить геогр. основу %1").arg("ph.load");
  }
  meteo::GeoPoint gp;
  gp.setLatDeg(0);
  gp.setLonDeg(0);

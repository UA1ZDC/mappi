...
//----------------------------------------------------------------------------
// Determine the Julian Day value at the present moment
//
double DateOps::now()
{
  time_t tt = time(0);
  struct tm* pt = localtime(&tt);
  long secs = pt->tm_hour * Astro::ISECONDS_PER_HOUR +
              pt->tm_min * Astro::ISECONDS_PER_MINUTE +
              pt->tm_sec;
  return dmyToDoubleDay( pt->tm_mday, pt->tm_mon+1, pt->tm_year + 1900, secs );
}

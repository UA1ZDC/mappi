...
UnimasServer::UnimasServer(AppStatusThread* status, StreamModule *p)
  :QTcpServer(p),
    stream_module_(p),
    status_(status)
{
...
UnimasServer::~UnimasServer()
{
  slotStreamFinihsed();
}

void UnimasServer::init(const msgstream::Options& options,
...
void UnimasServer::incomingConnection(qintptr socketDescriptor)
{
//  pauseAccepting();
  if(nullptr != sender_) {
  ...
  sender_ = new UnimasSender(socketDescriptor,opt_,outgoing_, incoming_, status_);
...
void UnimasServer::slotStreamFinihsed(){

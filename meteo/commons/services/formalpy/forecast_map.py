...
class ODPTable(MapFormal):
  ...
  def initFromProto(self, proto):
    ...
    if proto.map:
      ...
      # если указана заблаговременность прогноза
      if proto.map.hour:
        self.hour          = proto.map.hour
  ...
  def fillDocument(self):
    ...
    if self.stations is not False and self.stations is not None and len(self.stations)>0 :
      for _ in self.stations:
        ...
        date_with_forecast = self.arg_date + datetime.timedelta(hours=self.hour)
                
        title_context.set_text_content(     
            "Прогностическая карта погоды по территории {} на {}".format(
                _['title'], 
                date_with_forecast.strftime("%H:%M %d.%m.%Yг.")
            ) 
        ) 
  ...
  def createMap(self, dt,coords,scale=14):
    ...
    response     = self.ctrl.RemoteCall( srv.GetDocument, request, 30000 )

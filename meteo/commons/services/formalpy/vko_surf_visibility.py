...
kTemplate = PRJ_DIR + "/share/meteo/odtsamples/vko_visibility.odt"
kBaseName = 'vko_visibility_'

def createVisibility(args):
  doc = vkodoc.VkoDoc()
  ...
  if (ok):
    doc.fillheader()
    doc.fillOne(kVkoN, 'tableN', 'B', 5)
    doc.fillByMonth(kVkoh, 'tableh', 'B', 3)
    doc.fillByMonthHour(kVkohV, 'tablehV', 'B', 3)
    doc.fillOne(kVkoC, 'tableC', 'B', 2)
    doc.fillByMonth(kVkoV, 'tableV', 'B', 3)
    doc.save(outname)

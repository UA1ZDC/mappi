...
kTemplate = PRJ_DIR + "/share/meteo/odtsamples/vko_phenomena.odt"
kBaseName = 'vko_phenomena_'

def fillPhen(doc):
  try: 
    doc.calc.getClimatData(kVkow);
    res = doc.calc.calcTableValues()
    if (len(res) < 12):
      return
    idx = 0
    for phen in [0, 1, 2, 3, 4, 5]:
      rowI = 4 + phen*3
      one = [res[idx], res[idx + 1]]
      if not doc.fill(one, "tablew", 'B', rowI):
        break
      idx += 2
  except IOError:
...
def createPhen(args):
  ...
  if (ok):
    doc.fillheader()
    doc.fillOne(kVkoR, 'tableR', 'B', 3)
    doc.setImage('vkoR.png', 'imgR')
    doc.fillOne(kVkoYR, 'tableYR', 'B', 2)
    fillPhen(doc)
    doc.fillOne(kVkoss, 'tabless', 'B', 3)
    doc.fillOne(kVkoYss, 'tableYss', 'A', 3)
    doc.fillOne(kVkoYFr, 'tableYFr', 'A', 4)
    doc.save(outname)

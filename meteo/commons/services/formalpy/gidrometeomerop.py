...
# 
# Отчёт об участии в гидрометеорологическом обеспечении мероприятий оперативной подготовки
# 
class HudrometeoProp(DocumentFormal):

  def __init__(self):
    super().__init__()
    self.template_path     = "/share/meteo/odtsamples/gidrometeomerop.odt"
  ...
  def fillDocument(self):
    ...
    cell.set_value( self.arg_date.strftime("%d.%m.%Y") )

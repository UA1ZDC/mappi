    ...
    THEN
		...
		THEN
	        ...
	        query := FORMAT('DELETE FROM %s WHERE %s = ANY (SELECT %s FROM %s WHERE %s)',
				'meteo.' || fs, fs_key_field, msg_fs_field, msg, ts_interval);
			RAISE NOTICE '%', query;
	        EXECUTE query;

    	    GET DIAGNOSTICS fs_rec_count = ROW_COUNT;
	        RAISE NOTICE 'table: %, delete records: %', 'meteo.' || fs, fs_rec_count;
	        IF 0 < fs_rec_count
	        THEN
	            EXECUTE FORMAT('SELECT * FROM global.drop_table_partition($1)') INTO fs_part_count USING fs;
    	        RAISE NOTICE 'table: %, drop partition: %', 'meteo.' || fs, fs_part_count;
	        END IF;
		END IF;
    END IF;
